...
const generateContractPDF = async (contract) => {
  try {
    ...
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Base Price: ${(contract.pricing?.basePrice || 0).toLocaleString('vi-VN')} VND`, 20, 160);
    ...
    if (contract.pricing?.additionalServices && contract.pricing.additionalServices.length > 0) {
      ...
      contract.pricing.additionalServices.forEach((service, index) => {
        doc.text(`  ${service.service}: ${(service.price || 0).toLocaleString('vi-VN')} VND`, 20, yPos);
        yPos += 7;
      });
    }
    ...
    doc.text(`Total Price: ${(contract.pricing?.totalPrice || 0).toLocaleString('vi-VN')} VND`, 20, totalY + 7);
    doc.text(`Deposit: ${(contract.pricing?.deposit || 0).toLocaleString('vi-VN')} VND`, 20, totalY + 14);
    doc.text(`Balance Due: ${(contract.pricing?.balance || 0).toLocaleString('vi-VN')} VND`, 20, totalY + 21);
